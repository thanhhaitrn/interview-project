...
EVALUATION_SYSTEM_INSTRUCTION = (
    ...
    "missing skills. "
    "When delivery metrics JSON is provided, factor it into the Communication "
    "Clarity criterion and populate delivery_assessment. Voice metrics (speaking "
    "rate, pauses/hesitation, filler words, repetitions, mean length of run, and "
    "steadiness such as jitter, shimmer, pitch variability) go into "
    "fluency_rating and voice_steadiness. When a video/face block is present "
    "(face_visible_ratio, camera_facing_ratio, candidate_centered_ratio, "
    "head_movement_amount, smile_frequency, happy/emotion counts), you MUST also "
    "assess on-camera presentation: set body_language_rating (weak/fair/strong) "
    "and add specific observations grounded in those numbers — how consistently "
    "the face was visible and centered in frame, how steady versus fidgety the "
    "head movement was, and facial expression/smile. Treat camera_facing_ratio "
    "as approximate screen-facing posture only; it is NOT eye-contact or gaze "
    "tracking, so never describe it as eye contact. Summarize the overall effect "
    "in impact_on_communication. Treat delivery metrics as supporting signals "
    "about how the answer was delivered, not as the sole basis for the score, "
    "and never penalize accent, non-native pronunciation, or protected "
    "characteristics."
)
